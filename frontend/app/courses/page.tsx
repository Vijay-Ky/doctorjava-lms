...
export default function CoursesPage() {
  ...
  const [sort, setSort] = useState(searchParams.get('sort') || 'course_name,asc');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await lmsFetch<PageResult | Course[]>(`/api/courses?${params}`, {}, false);
    ...
    <PageShell>
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-5 md:px-8 md:py-10">
        <div className="rounded-3xl bg-gradient-to-r from-slate-950 via-slate-900 to-purple px-6 py-10 text-white sm:px-10">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-white/60">Courses</p>
          <h1 className="mt-2 max-w-xl font-display text-3xl font-bold sm:text-4xl">
            Learn job-ready skills online
          </h1>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-white/75">
            Browse courses like Udemy — curriculum, previews, assessments and certificates. Built for
            Doctor Java learners.
          </p>
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-4">
                ...
                <div className="mt-2 flex items-center gap-1 text-xs font-semibold text-amber-600">
                  ...
                  4.{(7 + (i % 3)).toString()}
                  <span className="font-normal text-slate-400">({120 + i * 37})</span>
                </div>
